...
echo "pre-submit bound admins: $(docker exec -i ams-backend-1 node - <<'EOF'
...
docker logs ams-backend-1 --since 1m 2>&1 | grep -iE 'car-actions|TelegramCarActions|offerApproval' | tail -4
