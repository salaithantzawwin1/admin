...
LID=$(docker exec ams-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT id FROM inventory_items WHERE balance <= \"minStock\" AND \"isActive\" LIMIT 1;"' | tr -d '\r\n')
...
curl -s -X POST -H "Authorization: Bearer $ATOKEN" -H 'Content-Type: application/json' \
  -d "{\"itemId\":\"$IID\",\"quantity\":5,\"unitPrice\":4500.50,\"reference\":\"E2E-PO-2\"}" "$BASE/inventory/restock" | docker exec -i ams-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);console.log('unitPrice:',j.transaction.unitPrice,'balanceAfter:',j.transaction.balanceAfter,'lastUnitPrice:',j.item.lastUnitPrice)})"

echo "--- 2) spending report (this month):"
curl -s -H "Authorization: Bearer $ATOKEN" "$BASE/inventory/spending" | docker exec -i ams-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);console.log('total:',j.total);j.items.forEach(i=>console.log(' ',i.code,i.name,'qty',i.qty,'avg',i.avgUnitPrice,'cost',i.cost,'noPrice:',i.noPrice))})"
...
docker exec ams-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT title, body FROM notifications WHERE type = '\''LOW_STOCK'\'' ORDER BY \"createdAt\" DESC LIMIT 3;"'
...
echo "--- 8) audit:"
docker exec ams-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT action FROM audit_logs WHERE module = '\''INVENTORY'\'' ORDER BY \"createdAt\" DESC LIMIT 3;"'
