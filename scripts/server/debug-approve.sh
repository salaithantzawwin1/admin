...
CREQ=$(printf '%s' "$CAR" | docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>console.log(JSON.parse(s).id))")
...
curl -s -H "Authorization: Bearer $STOKEN" "$BASE/fleet/drivers" | docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);const list=Array.isArray(j)?j:j.items;const d=list.find(x=>x.name==='U Zaw Zaw');console.log('employee field:',JSON.stringify(d?.employee))})"
echo "— cleanup:"
