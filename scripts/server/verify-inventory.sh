...
IID=$(docker exec ams-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT id FROM inventory_items WHERE balance > 2 AND \"isActive\" ORDER BY \"createdAt\" LIMIT 1;"' | tr -d '\r\n')
BAL0=$(docker exec ams-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT balance FROM inventory_items WHERE id = '\''$IID'\'';'" | tr -d '\r\n')
...
RID=$(echo "$RESP" | docker exec -i ams-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{try{console.log(JSON.parse(s).id)}catch(e){console.log('PARSE_FAIL:'+s.slice(0,80))}})")
...
BAL1=$(docker exec ams-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT balance FROM inventory_items WHERE id = '\''$IID'\'';'" | tr -d '\r\n')
DOC=$(docker exec ams-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT status FROM request_documents WHERE id = '\''$RID'\'';'" | tr -d '\r\n')
SUP=$(docker exec ams-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT status FROM office_supply_requests WHERE request_id = '\''$RID'\'';'" | tr -d '\r\n')
...
BAL2=$(docker exec ams-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT balance FROM inventory_items WHERE id = '\''$IID'\'';'" | tr -d '\r\n')
