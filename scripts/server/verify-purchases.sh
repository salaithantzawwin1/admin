...
curl -s -H "Authorization: Bearer $ATOKEN" "$BASE/inventory/purchase-totals" | docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{JSON.parse(s).forEach(i=>console.log(i.code,i.name,'| qty',i.qty,'| cost',i.cost,'| avg',i.avgUnitPrice,'| est:',i.estimated))})"
