...
J() { docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{try{const o=JSON.parse(s);console.log(eval('o'+process.argv[1]))}catch(e){console.log('PARSE_FAIL:'+s.slice(0,120))}})" "$1"; }
...
docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const o=JSON.parse(s);const u=o.items.find(x=>x.username==='employee1');console.log('total:',o.total,'| employee1 telegram:',JSON.stringify(u.telegram))})" < /tmp/users.json
...
echo "backend errors (5m): $(docker logs ams-test-backend-1 --since 5m 2>&1 | grep -ciE 'TSError|Cannot find|ERR_')"
echo "=== DONE ==="
