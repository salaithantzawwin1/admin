...
RID=$(docker exec ams-test-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT id FROM request_documents WHERE \"docNumber\" = '\''OSR-202609-0001'\'';"' | tr -d '\r\n')
BAL0=$(docker exec ams-test-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT balance FROM inventory_items WHERE id = '\''e14d904f-2ade-44b9-8469-1e42dc1e2065'\'';'" | tr -d '\r\n')
...
BAL1=$(docker exec ams-test-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT balance FROM inventory_items WHERE id = '\''e14d904f-2ade-44b9-8469-1e42dc1e2065'\'';'" | tr -d '\r\n')
...
echo "--- audit log:"
docker exec ams-test-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT action, username FROM audit_logs WHERE module = '\''INVENTORY'\'' ORDER BY \"createdAt\" DESC LIMIT 6;"'
