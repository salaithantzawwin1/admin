...
check() {
  ...
  local granted=$(curl -s -H "Authorization: Bearer $TOKEN" "$BASE/auth/me" | docker exec -i ams-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);console.log((j.permissions||[]).includes('suppliers.read'))})")
...
sleep 14
docker logs ams-backend-1 --since 1m 2>&1 | grep -E 'RBAC|migrations|listening' | head -4
