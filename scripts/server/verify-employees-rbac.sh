...
probe() {
  ...
  emp_read=$(curl -s -H "Authorization: Bearer $TOKEN" "$BASE/auth/me" | docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);console.log(((j.permissions||[]).includes('employees.read'))+' '+((j.permissions||[]).includes('departments.read')))})")
  local e d
