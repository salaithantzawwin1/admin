...
for ROLE in "$@"; do
  docker exec ams-test-db-1 psql -U "$PGU" -d "$PGD" -v code="$CODE" -v role="$ROLE" <<'SQL'
...
docker exec ams-test-db-1 psql -U "$PGU" -d "$PGD" -tAc "SELECT r.name FROM role_permissions rp JOIN roles r ON r.id=rp.\"roleId\" JOIN permissions p ON p.id=rp.\"permissionId\" WHERE p.code='$CODE' ORDER BY r.name"
