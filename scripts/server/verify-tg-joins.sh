...
printf 'INSERT INTO telegram_join_requests (id, "chatId", "tgUsername", "displayName", status, "updatedAt") VALUES (gen_random_uuid(), '\''777001'\'', '\''e2etguser'\'', '\''E2E TG User'\'', '\''PENDING'\'', now()) ON CONFLICT ("chatId") DO UPDATE SET status='\''PENDING'\'', "tgUsername"='\''e2etguser'\'', "updatedAt"=now();\n' | docker exec -i ams-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB' >/dev/null
JOINS=$(curl -s -H "Authorization: Bearer $STOKEN" "$BASE/settings/telegram/joins")
printf '%s' "$JOINS" > /tmp/joins.json
docker exec -i ams-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s).find(x=>x.chatId==='777001');console.log('join draft:',j?j.displayName+' @'+j.tgUsername:'MISSING')})" < /tmp/joins.json
JID=$(printf '%s' "$JOINS" | docker exec -i ams-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s).find(x=>x.chatId==='777001');console.log(j?j.id:'')})")
