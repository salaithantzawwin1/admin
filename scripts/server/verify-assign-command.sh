...
if docker exec ams-test-backend-1 sh -c "grep -c \"handleCommand\" /app/dist/cars/telegram-car-actions.service.js" 2>/dev/null | grep -qE '^[1-9]'; then
...
if docker exec ams-test-backend-1 sh -c "grep -c \"commands\" /app/dist/telegram/telegram.service.js" 2>/dev/null | grep -qE '^[1-9]'; then
