...
docker cp ams-test-backend-1:/tmp/t.png /tmp/t.png 2>/dev/null || true
...
docker exec ams-test-db-1 sh -c "psql -U \$POSTGRES_USER -d \$POSTGRES_DB -tAc 'SELECT \"imageStoredName\" FROM inventory_items WHERE id = '\''$IID'\'';'"
docker exec ams-test-backend-1 sh -c 'ls -la /app/uploads/item-* 2>/dev/null | tail -2'
...
echo "--- 7) audit:"
docker exec ams-test-db-1 sh -c 'psql -U $POSTGRES_USER -d $POSTGRES_DB -tAc "SELECT action FROM audit_logs WHERE action LIKE '\''%IMAGE%'\'' ORDER BY \"createdAt\" DESC LIMIT 3;"'
