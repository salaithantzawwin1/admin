...
VEH=$(curl -s -H "Authorization: Bearer $TTOKEN" "$BASE/fleet/vehicles" | docker exec -i ams-test-backend-1 node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>{const j=JSON.parse(s);const l=Array.isArray(j)?j:j.items;console.log(l.filter(v=>v.status==='AVAILABLE')[0].id)})")
